...
class EncounterForm extends React.Component {
 ...
 /**Prevents user from inputting non-digits and auto-splices in hyphens and periods at the appropriate positions
  * 
  * @param {*} e 
  */
 onBillingChange = (e) => {
    ...
    const seconddigits = digits.slice(4, 7)
    const thirddigits = digits.slice(8, 11)
    const fourthdigits = digits.slice(12, 13)
    const filtereddigits = firstdigits + seconddigits + thirddigits + fourthdigits
    if (e.target.value === '' || re.test(filtereddigits)) {
